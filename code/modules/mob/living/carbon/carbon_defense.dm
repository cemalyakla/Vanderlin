...
/mob/living/carbon/proc/try_knockout_teeth(damage, bclass, zone_precise, mob/living/attacker)
	...
	var/static/list/allowed_bclasses = list(BCLASS_BLUNT, BCLASS_SMASH, BCLASS_PUNCH)
	if(!(bclass in allowed_bclasses))
		return

	// Helmets or hard headgear covering the mouth fully prevent tooth loss.
